# SETUP — Backend Engineer (BE) | Memory Service

**Propósito:** Esto es lo que debes leer al iniciar sesión como Backend Engineer de Memory Service. No leas toda la carpeta `00-platform/`. Solo lo que dice acá.
...
git status   # debe mostrar branch wt-backend-be (idle) o branch de tarea activa
```

### Validación de entorno
```bash
test -d c:/Users/Martin/Documents/virtual-teams/memory-service/.vtt/worktrees/backend-be/ \
  && echo "Worktree OK" \
  || echo "ERROR: worktree backend-be no existe. Escalar a TL."
```
...
|---------|----------------------|
| `memory-service/.vtt/worktrees/backend-be/` | ✅ **PRIMARIO** — tu worktree |
...
| 3 | `c:/Users/Martin/Documents/virtual-teams/virtual-teams-setup/00-platform/01.operativos/OPERATIVO_BE_MEMORY-SERVICE.md` | Tu OPERATIVO específico del proyecto |
...
Los comandos exactos están en tu `OPERATIVO_BE_MEMORY-SERVICE.md` (PASO 1, archivo 3). Ejecutá:
...
1. **PASO 0** — `cd` a tu worktree (`.vtt/worktrees/backend-be/`) + validar que existe
...
**Fuente de verdad operativa:** `OPERATIVO_BE_MEMORY-SERVICE.md`
