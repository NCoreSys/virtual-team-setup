# SETUP — Database Engineer (DB) | Memory Service

**Propósito:** Esto es lo que debes leer al iniciar sesión como Database Engineer de Memory Service. No leas toda la carpeta `00-platform/`. Solo lo que dice acá.
...
```bash
cd c:/Users/Martin/Documents/virtual-teams/memory-service/.vtt/worktrees/backend-db
git status   # debe mostrar branch wt-backend-db (idle) o branch de tarea activa
```

### Validación de entorno
```bash
test -d c:/Users/Martin/Documents/virtual-teams/memory-service/.vtt/worktrees/backend-db/ \
  && echo "Worktree OK" \
  || echo "ERROR: worktree backend-db no existe. Escalar a TL."
```
...
> git worktree add ../.vtt/worktrees/backend-db -b wt-backend-db origin/main
> ```
...
|---------|----------------------|
| `memory-service/.vtt/worktrees/backend-db/` | ✅ **PRIMARIO** — tu worktree |
...
| 3 | `c:/Users/Martin/Documents/virtual-teams/virtual-teams-setup/00-platform/01.operativos/OPERATIVO_DB_MEMORY-SERVICE.md` | Tu OPERATIVO específico del proyecto |
...
| 5 | `c:/Users/Martin/Documents/virtual-teams/virtual-teams-setup/00-platform/06.Documentos_soporte/GUIA_MANIFEST_PARA_AGENTES.md` | Guía manifest (ejecutor) |
...
Los comandos exactos están en tu `OPERATIVO_DB_MEMORY-SERVICE.md` (PASO 1, archivo 3). Ejecutá:
...
1. **PASO 0** — `cd` a tu worktree (`.vtt/worktrees/backend-db/`) + validar que existe
...
**Fuente de verdad operativa:** `OPERATIVO_DB_MEMORY-SERVICE.md`
