...
**Propósito:** Esto es lo que debes leer al iniciar sesión como Design Lead de Memory Service. No leas toda la carpeta `00-platform/`. Solo lo que dice acá.
...
```bash
cd c:/Users/Martin/Documents/virtual-teams/memory-service/.vtt/worktrees/project-dl
git status   # debe mostrar branch wt-project-dl (idle) o branch de tarea activa
```

### Validación de entorno
```bash
test -d c:/Users/Martin/Documents/virtual-teams/memory-service/.vtt/worktrees/project-dl/ \
  && echo "Worktree OK" \
  || echo "ERROR: worktree project-dl no existe. Escalar a TL."
```
...
> git worktree add ../.vtt/worktrees/project-dl -b wt-project-dl origin/main
> ```
...
|---------|----------------------|
| `memory-service/.vtt/worktrees/project-dl/` | ✅ **PRIMARIO** — tu worktree |
...
| 3 | `c:/Users/Martin/Documents/virtual-teams/virtual-teams-setup/00-platform/01.operativos/OPERATIVO_DL_MEMORY-SERVICE.md` | Tu OPERATIVO específico del proyecto |
...
Los comandos exactos están en tu `OPERATIVO_DL_MEMORY-SERVICE.md` (PASO 1, archivo 3). Ejecutá:
...
1. **PASO 0** — `cd` a tu worktree (`.vtt/worktrees/project-dl/`) + validar que existe
...
**Fuente de verdad operativa:** `OPERATIVO_DL_MEMORY-SERVICE.md`
