# SETUP — DevOps Engineer (DO) | Memory Service

**Propósito:** Esto es lo que debes leer al iniciar sesión como DevOps Engineer de Memory Service. No leas toda la carpeta `00-platform/`. Solo lo que dice acá.
...
```bash
cd c:/Users/Martin/Documents/virtual-teams/memory-service/.vtt/worktrees/backend-do
git status   # debe mostrar branch wt-backend-do (idle) o branch de tarea activa
```

### Validación de entorno
```bash
test -d c:/Users/Martin/Documents/virtual-teams/memory-service/.vtt/worktrees/backend-do/ \
  && echo "Worktree OK" \
  || echo "ERROR: worktree backend-do no existe. Escalar a TL."
```
...
> git worktree add ../.vtt/worktrees/backend-do -b wt-backend-do origin/main
> ```
...
|---------|----------------------|
| `memory-service/.vtt/worktrees/backend-do/` | ✅ **PRIMARIO** — tu worktree |
...
| 3 | `c:/Users/Martin/Documents/virtual-teams/virtual-teams-setup/00-platform/01.operativos/OPERATIVO_DO_MEMORY-SERVICE.md` | Tu OPERATIVO específico del proyecto |
...
Los comandos exactos están en tu `OPERATIVO_DO_MEMORY-SERVICE.md` (PASO 1, archivo 3). Ejecutá:
...
1. **PASO 0** — `cd` a tu worktree (`.vtt/worktrees/backend-do/`) + validar que existe
...
**Fuente de verdad operativa:** `OPERATIVO_DO_MEMORY-SERVICE.md`
