...
| Código | Título | Path | Estado |
|---|---|---|---|
| `VTT.SKILL-DEV-001` | Registrar decisión en devlog | `02.normativa/03.Skills/dev/VTT.SKILL-DEV-001_decision.md` | ✅ v1.2 (2026-06-10) — bump por TW-OPS (VTS-028): drift IP corregido (`$VTT_BASE_URL` → `https://api.vttagent.com`, RULE-SEC-001) + H-2 documentado (severity ignorada BY-DESIGN en `decision` — omitir el campo) + acepta cualquier `categoryCode` del catálogo vivo de 12 valores (NO crear Skills DEV-006..010 — anti-pattern 1). Pertenece a `WORKFLOW-DEV-001.001`. |
| `VTT.SKILL-DEV-002` | Registrar observación en devlog | `02.normativa/03.Skills/dev/VTT.SKILL-DEV-002_observacion.md` | ✅ v1.2 (2026-06-10) — bump por TW-OPS (VTS-028): drift IP corregido (RULE-SEC-001) + H-2 documentado (severity ignorada BY-DESIGN en `observation` — omitir el campo) + acepta cualquier `categoryCode` del catálogo vivo. Pertenece a `WORKFLOW-DEV-001.001`. DEV-001 y DEV-002 son intercambiables operativamente. |
| `VTT.SKILL-DEV-003` | **Editar campos genéricos de un devlog entry** (PATCH `/devlog/:entryId`) | `02.normativa/03.Skills/dev/VTT.SKILL-DEV-003_edit_devlog.md` | ✅ v1.1 (2026-06-10) — bump por TW-OPS (VTS-028): drift IP corregido (RULE-SEC-001) + **§Error CRÍTICO H-3 BUG-CONSISTENCIA + R13 destacada** (DEV-003 NUNCA para cambiar `status` — usar SIEMPRE DEV-004; entry queda inconsistente si se pasa status en body) + tabla §Campos editables marca `status` como **EXCLUIDO**. Pertenece a `WORKFLOW-DEV-001.002`. |
| `VTT.SKILL-DEV-004` | **Lifecycle del devlog entry** (PATCH `/devlog/:entryId/status` — estados finales irreversibles) | `02.normativa/03.Skills/dev/VTT.SKILL-DEV-004_lifecycle_devlog.md` | ✅ v1.1 (2026-06-10) — bump por TW-OPS (VTS-028): drift IP corregido (RULE-SEC-001) + **§Caso 5 (`deferred`) ampliado con T2 BY-DESIGN + 3 workarounds + R14 destacada** (NUNCA usar `resolution` para preservar referencia al destino — backend limpia a null; usar description original / comment en tarea / `fixTaskId`). Único endpoint correcto para cambiar `status`. Pertenece a `WORKFLOW-DEV-001.002` y `WORKFLOW-DEV-001.003`. |
| `VTT.SKILL-DEV-005` | **Eliminar devlog entry** (DELETE `/devlog/:entryId` — destructivo, irreversible, sin soft-delete ni audit log) | `02.normativa/03.Skills/dev/VTT.SKILL-DEV-005_delete_devlog.md` | ✅ v1.1 (2026-06-10) — bump por TW-OPS (VTS-028): único cambio drift IP corregido (RULE-SEC-001). Reglas R1-R4 y árbol de decisión vigentes. Pertenece a `WORKFLOW-DEV-001.002` Paso 7. |
