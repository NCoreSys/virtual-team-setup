# METODOLOGÍA DE EJECUCIÓN DE SPRINTS V1.1

**Documento:** METODOLOGIA_EJECUCION_SPRINTS_V1.1.md
**Versión:** 1.1
**Fecha:** 2026-03-30  
**Autor:** PJM-Agent  
...
### 0.5 Contexto VTT: Sub-sprints MGP

El Motor de Gestión de Proyectos (MGP) de VTT se implementó en 5 sub-sprints. Esta es la razón por la que los campos `estimatedHours`, `complexity`, `category` y `dependsOn` son obligatorios:

| Sub-sprint | Funcionalidad | Campos que usa |
|------------|--------------|----------------|
| **5A: Plan + Gantt** | Algoritmo topológico, cálculo de días hábiles | `estimatedHours`, `plannedStartDate`, `plannedEndDate` |
| **5B: Time Tracking** | Actualiza `actualHours` automáticamente | `estimatedHours` (para comparar) |
| **5C: Métricas** | Burndown ideal vs real, forecast 3 escenarios | `estimatedHours`, `complexity`, `category` |
| **5D: Motor Inteligente** | BFS propagation, CPM, nivelación de recursos | `dependsOn`, `complexity`, `estimatedHours` |
| **5E: Alertas** | Overdue, critical_path, resource_overload | Todos los campos anteriores |

> **Si faltan campos:** El sistema rechaza la tarea (400 error) o las funcionalidades de planificación no funcionan correctamente.
...
Tarea formal donde DL verifica que la implementación FE coincide con los HTMLs diseñados y que el flujo de pantallas es correcto. Se ejecuta post-implementación FE.

**Formato de tarea en VTT:**
```
DL-REVIEW  Revisar implementación FE vs HTMLs + flujo pantallas  3h  MEDIUM  review
```
...
| Complejidad | MEDIUM |
| Categoría | review |
| Depende de | Todas las tareas FE del sprint completadas |

**Desviaciones encontradas:** Reportar como issues VTT con `taskId` de la tarea FE origen.
...
**QA-FLOW Scenarios:**
Escenarios específicos que verifican el flujo de navegación entre pantallas. Se definen en cada sprint según las pantallas implementadas.

| ID | Flujo | Pasa si... |
|----|-------|-----------|
| QA-FLOW-01 | Pantalla A → Pantalla B | Navegación automática correcta |
| QA-FLOW-02 | Step X → Step Y | Progress bar actualizado, datos persistidos |
| QA-FLOW-03 | Flujo completo del sprint | Todos los steps navegados sin error |
| QA-FLOW-04 | Resume draft en step intermedio | Navega al step correcto según estado guardado |
| QA-FLOW-05 | Pantallas vs HTML DL | Sin desviaciones críticas (coordinado con DL-REVIEW) |
| QA-FLOW-06 | Ruta directa inválida | Redirige al step correcto |

> **Nota:** Los IDs específicos se definen en HANDOFF_QA_ de cada sprint. Esta tabla es un ejemplo de escenarios típicos.
...
## 14. DELTA: EVOLUCIÓN V0 → V2

Esta tabla documenta los cambios entre la versión original del proceso (V0) y la versión actual (v2), para contexto histórico y trazabilidad.

| Cambio | V0 (original) | v2 (actual) |
|--------|---------------|-------------|
| **Handoffs por sprint** | 2 (TL + DL) | **4 (TL + DL + FE + QA)** |
| **Tareas con IDs formales** | ❌ | ✅ |
| **Horas explícitas por tarea** | ❌ | ✅ |
| **VTT Planning Data** | ❌ | ✅ (en los 4 handoffs) |
| **Dependencias formales (tabla FS)** | ❌ | ✅ |
| **DoD por rol** | ❌ | ✅ |
| **Gates de aprobación** | ❌ | ✅ |
| **Documentos dinámicos trackeados** | ❌ | ✅ |
| **Proceso de bugs via VTT issues** | ❌ | ✅ |
| **DevLog con "Issues encontrados" + "Cómo probar"** | ❌ | ✅ |
| **DL-REVIEW task formal (3h, MEDIUM)** | ❌ | ✅ |
| **QA-FLOW scenarios (flujos navegación)** | ❌ | ✅ |
| **Integration Audit por AR (separado de TL)** | TL hacía todo | ✅ **AR separado** |
| **Sprint Closure con 4 firmas formales** | implícito | ✅ **explícito** |
| **Secciones por handoff** | 10-11 | **19-20** |

### 14.1 Motivación de los Cambios

Los cambios principales surgieron de:

1. **Sistema MGP activo (Sprint 5A-5E):** Requiere campos obligatorios para que el motor de planificación funcione
2. **Equipo de 8+ agentes:** Cada rol necesita un documento de entrada consolidado
3. **Validación de implementación:** Mecanismo formal para verificar implementación vs diseño
4. **Separación de responsabilidades:** AR valida arquitectura, TL valida calidad técnica
