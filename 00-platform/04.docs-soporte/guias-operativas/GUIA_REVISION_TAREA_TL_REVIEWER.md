...
WORKTREE=".vtt/worktrees/backend-be"   # ajustar según rol
cd $WORKTREE
git log --oneline -3                    # debe mostrar commits de la tarea (feature/MS-XXX)
git branch --show-current               # debe ser feature/MS-XXX

# 2. Verificar que el clon base sigue intacto (no fue tocado)
cd ../../../memory-service-backend
git status                              # debe estar en main, sin cambios
git log --oneline -1                    # último commit debe ser de main, no del agente

# 3. Si hay execution_manifest, validar allowedPaths
cat .vtt/manifests/$TASK_ID.execution.json | python -m json.tool
# Cruzar con el diff del PR: archivos modificados deben estar dentro de allowedPaths
```

**Banderas rojas:**
- Si el clon base muestra cambios → agente trabajó en lugar incorrecto, rechazar
- Si el diff toca archivos fuera de `allowedPaths` → escalar a TL Ejecutor para revisión
- Si la branch no existe en el worktree del agente → algo está mal con el manifest
...
### Paso 16 — Cleanup branch local en worktree del agente (NUEVO v2.1)

> Aplica solo después de que el PR esté **mergeado en GitHub**.
> Deja el worktree del agente limpio (en `main`) listo para su próxima tarea.

```bash
# Mapear rol del agente → worktree (mismo mapeo que Paso 5b)
WORKTREE=".vtt/worktrees/backend-be"   # ajustar según rol

cd c:/Users/Martin/Documents/virtual-teams/memory-service/$WORKTREE

# 1. Actualizar main local con el merge del PR
git checkout main
git pull origin main

# 2. Borrar branch local de la tarea (ya está mergeada)
git branch -d feature/$TASK_ID

# 3. Verificar estado limpio
git status                              # debe estar en main, working tree clean
git log --oneline -1                    # último commit es el merge del PR

# 4. (Opcional) Limpiar execution_manifest de la tarea
mv ../../.vtt/manifests/$TASK_ID.execution.json \
   ../../.vtt/manifests/_archived/$TASK_ID.execution.json
```

**Si `git branch -d` falla con "branch not fully merged":**
- Significa que el agente tiene commits locales no pusheados. Investigar antes de forzar.
- Si confirmas que ya están en main vía squash merge: `git branch -D feature/$TASK_ID`

**Si el agente tenía cambios sin commitear (no debería pero por si acaso):**
- NO los borres con `--force`. Diagnosticar primero qué quedó pendiente.
