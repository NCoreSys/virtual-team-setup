...
- `POST /api/projects/d0fc276d-e764-4a83-96e9-d65f086ed803/trackable-items` — registrar ADR formal (`typeCode: ADR`)
...
3. **TrackableItems** creados o vinculados (ADRs si aplica — o N/A confirmado)
...
curl -s -X POST "http://77.42.88.106:3000/api/tasks/MS-XXX/criteria/{criteriaId}/fulfill" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"status":"met","evidence":"PR #N, diagrama generado","notes":"opcional"}'
```

### Crear ADR como TrackableItem
```bash
curl -s -X POST "http://77.42.88.106:3000/api/projects/d0fc276d-e764-4a83-96e9-d65f086ed803/trackable-items" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"typeCode":"ADR","title":"ADR-XXX: Título","description":"Decisión + razón","priority":"high"}'
```
...
8. Si ADR → escribirlo formal en `Release2.0/02-AR/ADR-XXX_*.md` + registrar como TrackableItem
9. Registrar CAs con `fulfill` + verificar Review Gate
10. Commit + push + PR
