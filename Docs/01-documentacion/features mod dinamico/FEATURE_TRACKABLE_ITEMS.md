...
```json
{
  ...
  "priority": "high",
  "status": "open",
  "originTaskId": "uuid-task-ms-023",
  "originRef": "MS-023, AMB-UX-01",
  "evidenceRef": "2.6.1 §1, 2.6.3 EF-01"
}
```
...
| `traces_to` | Trazabilidad hacia abajo | RF-001 traces_to US-001 |
| `tests` | Test case verifica item | TC-001 tests US-001 |
...
| Método | Ruta | Descripción |
|--------|------|-------------|
| GET | `/api/projects/:projectId/traceability-report` | Matriz de trazabilidad RF→US→AC→TC |
| GET | `/api/projects/:projectId/coverage-report` | Reporte de cobertura con métricas |
| GET | `/api/projects/:projectId/trackable-items/deferred` | Items diferidos |

#### Response de Coverage Report
...
```json
{
  ...
  "metrics": {
    "rfWithUS": { "total": 29, "covered": 29, "percent": 100 },
    "usWithAC": { "total": 33, "covered": 33, "percent": 100 },
    "usMustWithTC": { "total": 21, "covered": 19, "percent": 90, "target": 80 },
    "usTotalWithTC": { "total": 33, "covered": 26, "percent": 79 },
    "endpointsCovered": { "total": 11, "covered": 11, "percent": 100 }
  },
  "gaps": [
    {
      "id": "GAP-01",
      "description": "GET /timeline sin test suite dedicada",
      "severity": "minor",
      "recommendation": "QA crea TS-09 en Fase 5"
    }
  ],
  "status": "pass"
}
...
CREATE TABLE trackable_items (
  ...
  origin_task_id TEXT REFERENCES tasks(id),      -- Tarea donde se identificó
  origin_ref VARCHAR(255),                        -- Referencia libre (ej: "MS-023, AMB-UX-01")
  evidence_ref TEXT,                              -- Evidencia documental (ej: "2.6.1 §1, 2.6.3 EF-01")
...
**¿Para qué sirven originTaskId/originRef/evidenceRef?**
Permiten trazar el origen de un item:
- `originTaskId`: Vincula a la tarea donde se identificó (ej: tarea de análisis MS-023)
- `originRef`: Referencia libre al origen (ej: "MS-023, AMB-UX-01")
- `evidenceRef`: Referencia a documentos que lo soportan (ej: "2.6.1 §1, 2.6.3 EF-01")

Estos campos son opcionales pero mejoran la trazabilidad para auditoría.
