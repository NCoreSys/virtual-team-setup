...
```json
{
  ...
  "delivery": {
    ...
    "vtt_report_comment_id": "<id del comment SKL-REPORT-01>",
    "skl_report_01_full": "<texto literal del comment>",
...
[ ] Comment SKL-REPORT-01 POSTEADO en VTT (el manifest lo referencia literalmente)
[ ] AHORA SÍ: generar manifest v1.0 y subir
```

Si saltaste alguno → tu manifest tendrá campos `null`. Vuelve atrás.
...
| Generar manifest antes de postear SKL-REPORT-01 | `skl_report_01_full: null` | Postear comment PRIMERO, luego generar manifest |
