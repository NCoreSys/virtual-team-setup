...
5.2.13 **TL genera mensaje al agente y lo postea como comment en VTT** → **[ACTIVIDAD]** → invoca `SKL-MESSAGE-01` (legacy, futuro `VTT.SKILL-MSG-001`) — auto vía `scripts/gen_mensaje.py --post`
   - **Usa template:** `03.templates/tarea/TEMPLATE_MENSAJE_ASIGNACION.md` v2.0 (con sección Working Directory condicional WT/no-WT)
   - **Contenido obligatorio del mensaje:**
     ...
     - **Comando del script `gen_task_manifest.py`** que el agente ejecutará al cerrar (con todos los flags pre-rellenados)
...
> **Pendiente normativo (NO bloqueante hoy):** `SKL-MESSAGE-01` aún es legacy. Cuando se migre a `VTT.SKILL-MSG-001`, este paso apuntará a un Workflow propio `VTT.WORKFLOW-MSG-001.001`. Mientras tanto, el script vivo `gen_mensaje.py` cumple la función pero NO lee el `TEMPLATE_MENSAJE_ASIGNACION.md v2.0` formalmente — tiene el formato hardcodeado. Pendiente: refactor del script para que lea el template.
