#!/usr/bin/env python3
"""
Memory Service R1 — VTT Creation Script
=========================================
Crea en VTT desde cero:
  - 1 Project (Memory Service)
  - 10 Phases
  - 65 Deliveries
  - 116 Tasks (con metadata completa)
  - 15 Dependencies criticas

Salida:
  VTT_UUIDS_MEMORY_SERVICE.json con todos los UUIDs capturados.

Uso:
  export VTT_SERVICE_KEY=...
  python3 create_memory_service_vtt.py
...
OUTPUT_FILE = "VTT_UUIDS_MEMORY_SERVICE.json"

# ============================================================================
# UUIDs DE USUARIOS (Memory Service)
# ============================================================================

PM_UUID  = "350831b2-e1ae-4dbe-b2eb-7e023ec2e103"  # Product Manager
PJM_UUID = "0ff63a29-0bc0-465a-b9bd-5f71476bc91d"  # Project Manager
TL_UUID  = "92225290-6b6b-4c1f-a940-dcb4262507aa"  # Tech Lead
...
USERS = {
    ...
    "FE":  "d23c9cd9-a156-433b-8900-94add5488eec",  # Frontend Developer
...
PROJECT = {
    ...
    "description": (
        ...
        "Persiste conversaciones de 5 fuentes (CLI, Web, SDK, ChatGPT, VTT_CHANNEL), "
        "clasifica por reglas deterministicas, calcula costos y entrega contexto "
        "estructurado en <500ms al runtime. Puertos 3002 (API) y 3003 (UI). "
        "Integra con Runtime v1.1, Prompt Builder v1.3 y Hook Manager VTT."
    ),
...
PHASES = [
    ("Project Setup",     1, "Iniciacion: infra VM, repo, tooling, team onboarding, kickoff."),
    ("Discovery",         2, "Problem Definition + Value Proposition (feature interna VTT)."),
    ("Planning",          3, "Vision, scope, stakeholders, risks, timeline, budget."),
    ("Analysis",          4, "FR, NFR, use cases, user stories, business rules, flows, AC, traceability."),
    ("Design UX/UI",      5, "Personas, IA, design system, wireframes, mockups, handoff DL."),
    ("Design Technical",  6, "Solution arch, code arch, DB design, API design, sequences, ADRs, security, infra, estimates."),
    ("Development",       7, "S01-S06 backend + UI-01..04 frontend. 46 tareas."),
    ("Testing",           8, "Test planning, cases, environment, functional, integration, E2E, performance, security, UAT, bug fixes."),
    ("Deploy",            9, "Infra setup, CI/CD, staging, smoke, production, post-deploy monitoring, rollback."),
    ("Operations",       10, "Monitoring, support, hotfixes, improvements, security updates, scaling."),
]
...
DELIVERIES = [
    # Phase 1 (1)
    ("Project Setup", "Project Foundation Ready", 1, "Infra + repo + tooling + team + kickoff listos."),

    # Phase 2 (2)
    ("Discovery", "Problem Definition", 1, "Problem Statement + Pain Points + Current Solutions + Why Now + Validation."),
    ("Discovery", "Value Proposition", 2, "VPC + UVP + Key Differentiators + Target Customer + Value Hypothesis."),

    # Phase 3 (6)
    ("Planning", "Vision & Objectives", 1, "Vision, Mission, Goals, KPIs, North Star, OKRs."),
    ("Planning", "Scope",               2, "Scope Statement, In/Out-Scope, MVP Definition, Future Phases, Assumptions."),
    ("Planning", "Stakeholders",        3, "Stakeholder Map, Register, RACI, Communication Plan."),
    ("Planning", "Risks",               4, "Risk Register, Assessment, Mitigation, Contingency, Monitoring."),
    ("Planning", "Timeline",            5, "Schedule, Milestones, Phase Breakdown, Sprint Calendar, Dependencies, Critical Path, Buffer."),
    ("Planning", "Budget & Resources",  6, "Estimate, Cost Breakdown, Resource Plan, ROI, Tracking."),

    # Phase 4 (8)
    ("Analysis", "Functional Requirements",      1, "SRS + Requirements List + MoSCoW + Feature List + Decomposition + Approval."),
    ("Analysis", "Non-Functional Requirements",  2, "NFR doc + Performance (<500ms) + Security + Scalability + Availability + Usability."),
    ("Analysis", "Use Cases",                    3, "Use Case Doc + Diagram + List + Detailed + Actors + Relationships."),
    ("Analysis", "User Stories",                 4, "Product Backlog + Stories + Story Map + Epics + Estimation + Sprint Assignment."),
    ("Analysis", "Business Rules",               5, "Rules Doc + List + Validation + Calculation + Authorization + State Transition + Glossary."),
    ("Analysis", "User Flows",                   6, "Flow Diagrams + Happy Path + Error + Edge Cases + Journey Maps + Task Flows + Navigation."),
    ("Analysis", "Acceptance Criteria",          7, "AC Doc + Gherkin per Story + DoD + DoR + Test Scenarios."),
    ("Analysis", "Traceability Matrix",          8, "Matrix + RF->US + US->Test + Coverage Report."),

    # Phase 5 (6)
    ("Design UX/UI", "Personas",                   1, "Personas Doc + Cards + Primary/Secondary + Scenarios + Jobs to be Done."),
    ("Design UX/UI", "Information Architecture",   2, "Site Map + Navigation + Patterns + Content Inventory + Taxonomy + Menu + URL Structure."),
    ("Design UX/UI", "Design System",              3, "Tokens + Color + Typography + Spacing + Icons + Component Library + Docs + Pattern Library + Assets."),
    ("Design UX/UI", "Wireframes",                 4, "Low-fi + Mid-fi + Desktop wireframes para las 9 paginas + Annotations + Flows + Breakpoints."),
    ("Design UX/UI", "Mockups UI Design",          5, "Mockups completos desktop + Component States + Empty/Error/Loading States."),
    ("Design UX/UI", "Design Handoff",             6, "Handoff Doc + Specs Export + Asset Export + CSS Variables + Redlines. HITO para FE."),

    # Phase 6 (9)
    ("Design Technical", "Solution Architecture",  1, "Architecture Doc + C4 L1/L2/L3 + Tech Stack + Integration Points + Data Flow."),
    ("Design Technical", "Code Architecture",      2, "Folder Structure + Coding Standards + Design Patterns + Module Deps + Naming + Error Handling."),
    ("Design Technical", "Database Design",        3, "ERD (19 tablas+10 catalogos) + Prisma Schema + Table Specs + Index Strategy + Data Dict + Migration + Seed + Backup."),
    ("Design Technical", "API Design",             4, "OpenAPI + Endpoints (11 R1) + R/R Examples + Pagination + Error Codes + Auth + AuthZ + Rate Limit + Versioning + Postman + Guidelines."),
    ("Design Technical", "Sequence Diagrams",      5, "Doc + Auth Flow + Business Flows + Error Flows + Integration Flows + Async Flows."),
    ("Design Technical", "ADRs",                   6, "Template + Index + 48 ADRs individuales (D-MEM + D-INT) + Decision Log."),
    ("Design Technical", "Security Plan",          7, "Plan + AuthN + AuthZ + Data Protection + Encryption + OWASP + Headers + Secrets + Input Validation + Logging + Incident Response."),
    ("Design Technical", "Infrastructure Plan",    8, "Plan + Diagram (Hetzner) + Server Specs + Network + Env Matrix + Scaling + Backup + DR + Cost + SLA + Monitoring."),
    ("Design Technical", "Technical Estimates",    9, "Estimates + Story Points + Task Breakdown + Effort Matrix + Complexity + Risk-adjusted + Dependencies Map + Velocity + Capacity."),

    # Phase 7 (10)
    ("Development", "S01: Schema + Seeds",              1, "Schema Prisma (19 tablas+10 catalogos) + migraciones + partial indexes + GIN index + seed + Express setup + catalog cache."),
    ("Development", "S02: Import + Timeline",           2, "POST /import (4 fuentes) + POST /import-review + POST /upload + GET /timeline + error handling."),
    ("Development", "S03: Content + Context",           3, "GET /content (parse storage) + GET /context (<500ms fail-fast) + classifier deterministico + tests QA."),
    ("Development", "S04: Adapters + Cleanup",          4, "Adapter Claude Web + Adapter ChatGPT + storage writer + cleanup cron + status handler + tests."),
    ("Development", "S05: Lista + Cost + Dashboard",    5, "GET /conversations + cost-reports + /dashboard/stats + /health + integration tests."),
    ("Development", "S06: Docker + Integration",        6, "Dockerfile + docker-compose + CI config + env vars + integracion Hook Manager + E2E Runtime/PB."),
    ("Development", "UI-01: Setup + Timeline + Viewer", 7, "Setup React+Vite+Tailwind + routing + Timeline page + Conversation Viewer TASK + Auth context."),
    ("Development", "UI-02: Dashboard + Cost + Import", 8, "Dashboard page + Cost Report Proyecto + Import Manual."),
    ("Development", "UI-03: Viewer REVIEW + Lista",     9, "Lista conversaciones + Component AGENT_REVIEW multi-agente."),
    ("Development", "UI-04: Cost Agente + Health",     10, "Cost Report Agente + Health page + Polish responsive desktop."),

    # Phase 8 (10)
    ("Testing", "Test Planning",          1, "Test Plan v1.0 + Strategy + Scope + Schedule + Resource Allocation."),
    ("Testing", "Test Cases",             2, "Test Cases Document con TC-001..TC-N + Test Data + Expected Results."),
    ("Testing", "Test Environment",       3, "Environment + Test BD + Seeding + Documentation."),
    ("Testing", "Functional Testing",     4, "Results + Execution Log + Defects + Pass/Fail + Evidence."),
    ("Testing", "Integration Testing",    5, "Suite Jest+supertest + Results + API Contract Tests + Coverage."),
    ("Testing", "E2E Testing",            6, "Suite Playwright + Results + Critical Path + Visual Regression + Docs."),
    ("Testing", "Performance Testing",    7, "Load + Stress + Performance Metrics + Bottleneck Analysis. VALIDA SLA <500ms."),
    ("Testing", "Security Testing",       8, "Security Plan + Penetration Test + Vuln Scan + OWASP + Findings + Remediation + Sign-off."),
    ("Testing", "UAT",                    9, "UAT Plan + Test Cases + Results + User Feedback + Sign-off (equipo VTT interno)."),
    ("Testing", "Bug Fixes",             10, "Bug Fixes Implemented + Regression Tests + Resolution Report."),

    # Phase 9 (7)
    ("Deploy", "Infrastructure Setup",    1, "Infrastructure Ready + Servers + Network + Security Groups + LB + DB + Storage + SSL."),
    ("Deploy", "CI/CD Configuration",     2, "CI Pipeline + CD Pipeline + Build Scripts + Deploy Scripts + Env Configs + Documentation."),
    ("Deploy", "Staging Deploy",          3, "Staging Deploy + URL + Migration Run + Health Check."),
    ("Deploy", "Smoke Testing",           4, "Smoke Test Results + Critical Paths + Sign-off."),
    ("Deploy", "Production Deploy",       5, "Prod Deploy + URL + DNS + SSL + Release Notes + Deployment Log."),
    ("Deploy", "Post-Deploy Monitoring",  6, "Dashboard Grafana + Alerts + Log Aggregation + Metrics + Error Tracking + Post-Deploy Report 24h."),
    ("Deploy", "Rollback Plan",           7, "Rollback Plan + Scripts + Tested + Runbook + Decision Criteria."),

    # Phase 10 (6)
    ("Operations", "Monitoring",                1, "Uptime + Performance + Error + Weekly Reports."),
    ("Operations", "User Support",              2, "Support Process + Ticket System + SLA + Support Metrics."),
    ("Operations", "Bug Fixes Operations",      3, "Hotfix Process + Releases + Bug Tracking."),
    ("Operations", "Incremental Improvements",  4, "Minor Releases + Feature Flags + A/B Tests + Improvement Backlog."),
    ("Operations", "Security Updates",          5, "Security Patches + Dependency Updates + Security Audits + Vulnerability Reports."),
    ("Operations", "Scaling",                   6, "Scaling Reports + Capacity Planning + Auto-scaling Config + Cost Optimization."),
]
...
def post(path: str, body: dict[str, Any], token: str) -> dict[str, Any]:
    """POST con Authorization Bearer. Retorna payload JSON del response."""
    req = urllib.request.Request(
        f"{API_URL}{path}",
...
def extract_id(response: dict[str, Any]) -> str:
    """Extrae id/UUID del payload (intenta data.id, luego id raiz)."""
...
def main() -> None:
    log(f"Memory Service R1 — VTT Creation Script")
    log(f"API: {API_URL}")
